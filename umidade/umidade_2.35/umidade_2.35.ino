...
int analogSoloMolhado = 200; //VALOR MEDIDO COM O SOLO MOLHADO (VOCÊ PODE FAZER TESTES E AJUSTAR ESTE VALOR)
...
void loop() {
  ...
  int Percentual = map(ValAnalogIn, 1023, 0, 0, 200); // Relaciona o valor analógico à porcentagem
  ...
  if (dht.readTemperature() >= 27.0) {
    ...
    int Percentual = map(ValAnalogIn, 1023, 0, 0, 200);
    ValAnalogIn = constrain(analogRead(pinoAnalog), analogSoloMolhado, analogSoloSeco);
    Percentual = map(ValAnalogIn, analogSoloMolhado, analogSoloSeco, percSoloMolhado, percSoloSeco);
    ...
    while (analogRead(pinoAnalog) >= 300) {
