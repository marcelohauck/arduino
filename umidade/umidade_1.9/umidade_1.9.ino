...
#define pinoAnalog1 A1 // Define o pino A1 como "pinoAnalog1"
...
int Percentual = map(ValAnalogIn, 1023, 0, 0, 350); // Relaciona o valor analógico à porcentagem
ValAnalogIn1 = analogRead(pinoAnalog1); // Relaciona o valor analógico com o recebido do sensor
int Percentual1 = map(ValAnalogIn1, 1023, 0, 0, 350); // Relaciona o valor analógico à porcentagem
...
while (analogRead(pinoAnalog) >= 450); 
...
}else if (analogRead(pinoAnalog1) >= 850){  
 
while (analogRead(pinoAnalog1) >= 450);
...
delay (60000); // Estabelece o tempo de 1s para reinicializar a leitura
